...
interface RestaurantJson {
  ...
  tags?: string[]
  photo_url?: string
  category_place?: string // venue_category enum: restaurants, bars-nightlife, shopping, hebergement, sport-bien-etre
}
    ...
    try {
      for (const restaurant of restaurants) {
        try {
          ...
          // Determine place_category from JSON (category_place field)
          // Valid values: restaurants, bars-nightlife, shopping, hebergement, sport-bien-etre
          // Default to 'restaurants' if not provided (for backward compatibility)
          const validPlaceCategories = ['restaurants', 'bars-nightlife', 'shopping', 'hebergement', 'sport-bien-etre']
          const placeCategory = restaurant.category_place && validPlaceCategories.includes(restaurant.category_place)
            ? restaurant.category_place
            : 'restaurants'
                    ...
                    return (
                      ...
                      <td className="px-4 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                          placeCategory === 'restaurants' ? 'bg-orange-100 text-orange-800' :
                          placeCategory === 'bars-nightlife' ? 'bg-purple-100 text-purple-800' :
                          placeCategory === 'shopping' ? 'bg-blue-100 text-blue-800' :
                          placeCategory === 'hebergement' ? 'bg-green-100 text-green-800' :
                          placeCategory === 'sport-bien-etre' ? 'bg-teal-100 text-teal-800' :
                          'bg-gray-100 text-gray-800'
                        }`}>
                          {placeCategory === 'restaurants' ? 'Restaurants' :
                           placeCategory === 'bars-nightlife' ? 'Bars & Nightlife' :
                           placeCategory === 'shopping' ? 'Shopping' :
                           placeCategory === 'hebergement' ? 'Hébergement' :
                           placeCategory === 'sport-bien-etre' ? 'Sport & Bien-être' :
                           placeCategory}
                        </span>
                        {!restaurant.category_place && (
